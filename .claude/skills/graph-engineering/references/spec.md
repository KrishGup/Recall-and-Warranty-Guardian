...
defaults: { model: sonnet, effort: low, bridge: bedrock, failure: { retries: 1, timeout_ms: 300000 } }
nodes: [ ... ]
```
...
Prompts are templates: `{{ input.topic }}`, `{{ $nodes.a.output.x }}`, `{{ json item }}` (forces JSON). Non-string values render as pretty JSON. Unknown `{{ }}` is left untouched (safe for JSON examples in prompts). Template references without `$` (`{{ nodes.a.output.x }}`) are dependencies too.
...
- id: research
  kind: agent
  model: haiku            # haiku | sonnet | opus | fable | full model id (or a Bedrock model id)
  effort: low             # low | medium | high | xhigh | max
  bridge: bedrock         # optional override: bedrock | anthropic | claude-code | inbox | mock
...
```
Output is validated against `output_schema`; invalid output triggers a repair call (the model sees the validation errors) up to `repair_attempts`, then counts as a failed attempt. On `bedrock`/`anthropic` the call is a Strands `Agent` with `structured_output_model` (the schema becomes a Pydantic model; tools run inside the Strands agent loop). On `claude-code`/`inbox`/`mock` it is one structured call.
...
- id: dedupe
  ...
  module: ./reducers/custom.py     # a Python module exporting reduce(input, args, ctx) -> output (sync or async)
  input: { items: $nodes.flat.output.items }
  args: { key: [claim] }
  output_schema: {...}             # optional
  side_effect: true                # irreversible: needs requires_gate, runs at most once per run
```
Built-ins: `flatten, filter_nulls, dedupe, sort, top_k, group_by, count_votes, normalize_labels, filter, pick, merge, identity, coverage, template, check_fields, classify_regex, record`. Reducers report `_stats: {in, out}` so the compression metric is real. `ctx` has `run_id`, `node_id`, `log(msg)`.
...
- id: verify
  ...
  tools: [WebFetch]                    # a verifier that must OPEN the source; cwd/max_turns/max_cost_usd as for agents
...
```
Fixed verifier output: `{ verdict: pass|kill, reasons: [], confidence: 0..1 }`. Results: `$nodes.verify.survivors`, `.killed`, `.kill_rate`, `.output.total`. A verification that did not execute is a kill (never mark passed unless it ran). The repair cycle is a conditional back-edge in the compiled Strands graph; nodes downstream of the producer wait for the fresh generation before they run again.
...
```
Nodes declare `requires_gate: approve`; they are structurally unreachable until the approval record exists. The gate is a Strands interrupt: the run pauses (`gren approve <run> <gate>`, dashboard, or MCP `gren_approve`) and resumes from the checkpoint.
...
## Providers (`bridge:`)
| provider | what executes the call | needs |
|---|---|---|
| `bedrock` | Strands `BedrockModel`: Claude on Amazon Bedrock, full Strands agent loop, tools from `strands_tools` | AWS credentials (env, profile, or an IAM role); `AWS_REGION` |
| `anthropic` | Strands `AnthropicModel`: the Anthropic API with the same agent loop | `ANTHROPIC_API_KEY` |
| `claude-code` | one headless Claude Code session per call (`claude -p --json-schema`), isolated context, optional tools, cost reported by the CLI | a Claude Code login (`claude auth login`) |
...
| `mock` | schema-driven deterministic output with latency/failure injection | none (`GREN_ALLOW_MOCK=1` to make it the default) |

Default provider: `--bridge` > `defaults.bridge` in the spec > `GREN_BRIDGE` > `anthropic` if `ANTHROPIC_API_KEY` is set > `bedrock` if AWS credentials exist > `claude-code`. Model aliases: `haiku`, `sonnet`, `opus`, `fable`; on Bedrock they map to cross-region inference profiles (`global.anthropic.<model>`), override with `GREN_BEDROCK_<ALIAS>`.

## Resume semantics
`gren resume <run_id>` continues from the checkpoint: nodes that were running are re-run (completed fan-out items are kept), a node that failed with `on_failure: block` is re-run with fresh retries, skips that cascaded from that failure are re-evaluated, and a `side_effect` node that was interrupted mid-execution is marked failed and never re-run automatically. Waiting gates pick up any approval recorded while the run was paused. Completed nodes replay instantly inside the compiled Strands graph.

## Developer loop: fork
`gren fork <run_id> --from <node[,node]> [--spec edited.yaml] [--input JSON]` (MCP: `gren_fork`, dashboard: "Fork run from here") copies the run, resets the named nodes and everything downstream, and re-executes only that part - upstream outputs (the expensive research fan-out, say) are reused. Iterate on a late prompt without paying for the whole graph again.

## Run state (`runs/<id>/`)
`run.json` (spec snapshot, input, status, totals, approvals, decisions), `state.json` (node checkpoint), `events.jsonl`, `artifacts/` (prompts + raw outputs per attempt), `inbox/` (tasks + results), `approvals/`, `nested/` (loop rounds, subgraphs).

## Metrics (`gren metrics <id>`)
critical-path latency (actual), sum of work, parallel speedup, peak width vs budget, node failure rate, retry rate, verifier kill rate, fan-out efficiency (unique/worker), compression ratio, human intervention (with waiting time separated from execution time), cost by model, hints.
