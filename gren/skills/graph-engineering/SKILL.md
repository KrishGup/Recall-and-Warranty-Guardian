...
description: Design, validate, run and monitor multi-agent workflows as explicit graphs with gren (nodes with one job, edges that carry data, deterministic reducers, adversarial verifiers, human gates, bounded loops, budgets) on Strands Agents / AWS Bedrock. Use when asked to build a multi-agent pipeline/workflow/graph, orchestrate agents, fan out research or review across workers, add verification or human approval to an agent flow, or run/monitor a gren graph. Also use when the user says "graph engineering", "gren", "orchestrator", "map-reduce agents", "escalation ladder", "tournament", or "discovery loop".
---
...
gren is the runtime: the `gren` CLI, the `gren ui` dashboard, and the `gren` MCP server (tools prefixed `gren_`). It compiles each graph into a Strands Agents graph and runs the model calls through Strands providers (Amazon Bedrock, the Anthropic API), headless Claude Code, or this session (inbox). Use the MCP tools when they are available in this session. If they are not, use the CLI: `gren <command>` (or `python -m gren <command>` with the project's interpreter).
...
- custom reducers are Python modules (`module: ./reducers/name.py` exporting `reduce(input, args, ctx)`); keep them deterministic and report `_stats: {in, out}`
...
## 4. Run - choose the provider

| situation | provider (`bridge`) |
|---|---|
| AWS credentials are configured (the AWS-native default) | `bedrock` |
...
| this Claude Code session should do the work with its own subagents (no key needed) | `inbox` - follow the orchestrator loop below |
...
### Orchestrator loop (provider `inbox`)
