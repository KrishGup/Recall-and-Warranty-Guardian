...
- **Dashboard** (`gren ui`): a light, Lucidchart-style canvas — pan, zoom, drag nodes, click to highlight upstream/downstream, hover an edge to see exactly what data crosses it, critical path overlay, nested-run drill-down; node inspector with prompts, raw outputs, attempts, killed candidates and reasons, gate approval with plain "if you approve / if you reject" lines, and "fork run from here"; drawer with live events, metrics, decisions (with the state that produced them), inbox task console, analysis, spec and output. **Design mode**: YAML editor with live validation, preview of the derived graph, save, and run.
...
| `release-pipeline.yaml` | **multi-app**: incident text → locate the code (Read/Grep) → baseline tests (code) → isolated **git worktree** (code) → implement the fix (Edit/Write in the worktree) → diff (code) → tests (code) → adversarial diff review (repair → implement) → changelog/PR text → **gate** commit → **gate** push + draft PR via `gh` → **gate** chat webhook (Slack/Discord/Zapier). Nothing irreversible happens without its own approval; without a remote or webhook URL the last two record dry runs | chain with explicit state, verify+repair, three gated side effects |
