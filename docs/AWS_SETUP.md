...
**This account is gated one level above the form (found 2026-09-13 ~23:00 UTC).** Every model on Bedrock, Anthropic or not (Nova, Llama, gpt-oss too), in every region, answers `Operation not allowed`; `get-foundation-model-availability` reports `authorizationStatus: NOT_AUTHORIZED` for all of them, and the use case form itself (console and `aws bedrock put-use-case-for-model-access`) answers "Your account is not authorized to perform this action. Please create a support case with details about your use case." That is AWS's Bedrock authorization review for accounts that never used Bedrock; no console setting bypasses it. The way through is a Support Center interaction (Basic plan is enough; the new "Support interactions" chat creates the case): describe the account, the errors above, the hackathon use case and the expected spend, and ask for Bedrock model invocation to be authorized (the AgentCore quotas below can go in the same case). **Case 178934068800881** ("Bedrock model invocation not authorized for account 779457758734") was opened 2026-09-13 19:04 EDT as Account and billing → Account → Other Account Issues, severity General question, Web contact (AWS promises an answer within 24 hours; replies arrive at the root email and in Support Center → Your support cases). Until it is granted, run the demo on the Anthropic API instead: `ANTHROPIC_API_KEY` in `.env` (gren picks the `anthropic` bridge), or for the server `GUARDIAN_DEPLOY_BRIDGE=anthropic` plus the key, then `python scripts/deploy_ec2.py update`. The hackathon requires Strands Agents, not Bedrock.
