...
const header = (name: string, description: string) => `name: ${name}
version: 1
...
export const SHAPES: Shape[] = [
  {
    ...
    B   C   D
     \\  |  /
        E`,
    use: "research, audits, batch analysis, competitive scans",
    template: (name) => `${header(name, "Fork/Join: plan -> parallel workers -> deterministic reduce -> synthesize")}output: { from: synthesize }
nodes:
  - id: plan
    kind: agent
    prompt: |
...
medium check
    | still uncertain?
strong model / human`,
    use: "most cases are easy but a few deserve expensive reasoning",
    template: (name) => `${header(name, "Escalation ladder: regex -> haiku -> sonnet -> human gate, routed by confidence")}output: { from: decision }
nodes:
  - id: rung0
    kind: code
    fn: classify_regex
...
candidate 3 -/`,
    use: "copy, designs, plans, code approaches, hypotheses",
    template: (name) => `${header(name, "Tournament: N candidates -> M independent judges -> deterministic vote count -> winner")}output: { from: winner }
nodes:
  - id: candidates
    kind: agent
    model: haiku
...
`,
  ...
  {
    id: "map-reduce-verify",
    title: "Map -> Reduce -> Verify -> Synthesize",
    diagram: `many workers
     |
...
attack weak findings (verify, kill authority)
     |
final answer`,
    use: "decision-grade research and large-scale review",
    template: (name) => `${header(name, "Map -> deterministic reduce -> adversarial verify -> synthesize")}output: { from: synthesize }
...
`,
  ...
  {
    id: "discovery-loop",
    title: "Bounded Discovery Loop",
    diagram: `search -> new findings? -> verify -> add to seen -> search again
stop after: no new findings for N rounds | max rounds | max spend | max time`,
    use: "when you do not know how large the problem is before starting",
    template: (name) => `${header(name, "Bounded discovery loop: search until no new findings for 2 rounds (max 5), dedupe against everything seen")}output: { from: discover }
nodes:
  - id: discover
    kind: loop
...
export function scaffold(shape: string, name: string): string {
  const s = SHAPES.find((x) => x.id === shape);
  if (!s) throw new Error(`unknown shape "${shape}". Known: ${SHAPES.map((x) => x.id).join(", ")}`);
