...
- **Strands Agents SDK.** gren compiles every YAML graph onto the [Strands Agents](https://strandsagents.com) `Graph`: each node becomes a Strands `MultiAgentBase` executor built through a `GraphBuilder`; agent nodes run a Strands `Agent` with `structured_output_model`; the household gate is a Strands interrupt (`event.interrupt(...)` from a `BeforeNodeCallEvent` hook) that survives a process exit. Both `guardian` and `gren` depend on `strands-agents` directly (see `pyproject.toml` and `gren/pyproject.toml`); `gren` also depends on `strands-agents-tools`. Guardian itself never imports `strands` — it only calls into gren, which owns every Strands touchpoint. `ARCHITECTURE.md` section 1 has the full node-by-node mapping.
- Python 3.11 or later. The venv is `.venv` (`.venv/Scripts/python` on Windows). Install with `pip install -e "./gren[dev]" -e ".[dev]"`. Run the tests with `GREN_ALLOW_MOCK=1 python -m pytest -q -p no:cacheprovider` (50 tests, mock provider, recorded feeds, about 30 s).
...
- The dashboard is a PWA (`vite-plugin-pwa` in `web/vite.config.ts`, `generateSW` mode). The manifest, `sw.js`, and the icons land in `web/dist` on `npm run build`; the SPA handler in `guardian/api/app.py` serves `sw.js`, `workbox-*.js`, the manifest, and `index.html` with `cache-control: no-cache`, and answers unknown `/api/...` GETs with a JSON 404 instead of the shell. The worker precaches the shell and caches the Google Fonts; it never touches `/api` or `/gren`, and `navigateFallbackDenylist` keeps `/api`, `/gren`, and any `?token=` visit on the network (the token visit sets the auth cookie). `registerType: 'prompt'`: `web/src/app/pwa/PwaStatus.tsx` shows the Reload bar and the offline notice on both surfaces (styles `.g-pwa*` in `theme/global.css`). No service worker in `npm run dev`; test the app behavior on the built site. `python scripts/build_pwa_icons.py` redraws the PNG icons from the favicon mark. `tests/test_api.py::test_dashboard_hosting_serves_the_pwa_entry_points` covers the hosting.
...
- `guardian serve` starts a background scheduler (nightly sweep, Gmail sync) unless you pass `--no-schedule`. The EC2 unit runs with `--no-schedule`; the systemd timer is the scheduler on the instance. `guardian seed` also seeds two case-study decisions and label photos; `--no-case-studies` seeds the 12 real items only.
- Demo material: `scripts/build_deck.py --export` writes `docs/deck/Guardian-Demo-Deck.pptx` and the PNG previews from `scripts/deck_content.py` (copy, speaker notes, timing). `scripts/record_demo.py all --reset` renders the slides, drives the live site with Playwright (its own Chromium), and assembles `var/demo/guardian-demo.mp4` with `docs/DEMO_SCRIPT.md`. The reset seeds the household without the case-study fixtures.
