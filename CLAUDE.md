...
- Python 3.11 or later. The venv is `.venv` (`.venv/Scripts/python` on Windows). Install with `pip install -e "./gren[dev]" -e ".[dev]"`. Run the tests with `GREN_ALLOW_MOCK=1 python -m pytest -q -p no:cacheprovider` (49 tests, mock provider, recorded feeds, about 30 s).
