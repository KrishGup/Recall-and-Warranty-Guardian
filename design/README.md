...
**Logo (option 1d, animated).** The mark is an SVG in a 48-unit box (`web/src/ui/Logo.tsx`, `web/public/favicon.svg`; the 1024 px renders are in `design/avatars`). Inside a `#000F08` disc: the status dot at (12, 34) with radius 5, and a `#F7F4F3` lid, the same circle moved aside. Closed, the lid rests at (29.8, 24), which reads as a thin crescent. Open, it rests at (38, 16), like a manhole lid swung aside, and shows the dot. A 1 px `#F7F4F3` hairline (`box-shadow`) separates the disc from the dark top bar; use no thicker ring. The lid opens 400 ms after the load and stays open when the status is not idle. The dot color follows the agent status and pulses when the agent works. The lid moves with `transform .8s cubic-bezier(.2,.8,.2,1)`.
